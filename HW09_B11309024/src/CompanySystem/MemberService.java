		...
		try {
			if (name_file.isDirectory()) {
				...
				for(String directoryname:directory) {// 逐一處理每個檔案
					input = new ObjectInputStream(new FileInputStream("C:\\data\\"+dep+"\\"+directoryname));
					try{
						while(true) { // 讀取檔案中所有的留言物件
						//強制轉型(要先做)
						if(directoryname.startsWith("1")) {
							obj = (Manager) input.readObject();
							System.out.println("主管:"+obj.getStaffName());
						}
						else if(directoryname.startsWith("0")){
							obj1 = (GeneralStaff) input.readObject();
							System.out.println("一般員工:"+obj1.getStaffName());
						}
						}
					}catch(EOFException eof){
